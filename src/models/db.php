...
class Dishes{
	...
	public function createDish($name, $price, $descriptions, $status, $image, $lastUpdatedByAdmin){
        ...
        public function updateDishById($id, $name, $price, $description, $status, $image, $admin_id){
            $query = "UPDATE `Dishes` SET `name`= '$name', `price`= '$price', `descriptions`= '$description', `status`= '$status', `image`= '$image', `lastUpdatedByAdmin`= '$admin_id' WHERE `id` = '$id'";
            $result = $this->db->query($query);
            return $result;
        }
